...
def init(inf, dIP)
	...
	@dstMAC = "78:2b:cb:a3:d9:e0"	# Manually set MAC, below is SUPPOSE to work - YMMV
...
init(inf, dIP)
	...
	@packet = PacketFu::Packet.parse(packet) 
		# Changed for lab machines
		dns = @packet.payload[2].unpack('h*')[0].chr + @packet.payload[3].unpack('h*')[0].chr  # Look for query
		if dns == '10'
...
# iptables -A FORWARD -p udp -s 192.168.0.18 --dport 53 -j DROP
